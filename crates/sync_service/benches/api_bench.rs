...
use serde_json::json;
use core_domain::entities::{Contact, User, Project};
use uuid::Uuid;
use chrono::Utc;
...
fn bench_contact_serialization(c: &mut Criterion) {
    ...
    c.bench_function("serialize_contact", |b| {
        b.iter(|| {
            serde_json::to_string(black_box(&contact)).unwrap()
        })
    });
    ...
    c.bench_function("deserialize_contact", |b| {
        b.iter(|| {
            serde_json::from_str::<Contact>(black_box(&json_str)).unwrap()
        })
    });
}
...
fn bench_json_operations(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        let contacts: Vec<Contact> = (0..*size).map(|i| Contact {
            id: Uuid::new_v4().to_string(),
        ...
        group.bench_with_input(
            ...
            &contacts,
            |b, contacts| {
                b.iter(|| serde_json::to_string(black_box(contacts)).unwrap())
            }
        );
...
fn bench_uuid_operations(c: &mut Criterion) {
    ...
    c.bench_function("uuid_from_string", |b| {
        b.iter(|| {
            Uuid::parse_str(black_box(&uuid_str)).unwrap()
        })
    });
}
...
fn bench_tag_filtering(c: &mut Criterion) {
    let contacts: Vec<Contact> = (0..1000).map(|i| Contact {
        id: Uuid::new_v4().to_string(),
        first_name: format!("User{}", i),
        last_name: Some("Test".to_string()),
        email: Some(format!("user{}@example.com", i)),
...
fn bench_data_cloning(c: &mut Criterion) {
    let contact = Contact {
        ...
        social_handles: vec![],
        tags: vec!["vip".to_string(), "engineering".to_string(), "manager".to_string()],
    ...
    c.bench_function("clone_contact", |b| {
        b.iter(|| {
            black_box(&contact).clone()
        })
    });
}
